<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <%@ taglib uri="http://www.springframework.org/tags" prefix="spring"%> 
<%@ taglib uri="http://www.springframework.org/tags/form" prefix="form"%> 
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>Meu CRUD Simples</title>
<link href="http://cdn.jsdelivr.net/webjars/bootstrap/3.3.7/css/bootstrap.css"
      href="@{/webjars/bootstrap/3.3.7/css/bootstrap.css}" rel="stylesheet" media="screen"/>
</head>
<body>
	<div class="container" style="margin-top: 40px">
		<div class="col-md-4">
			<a class="btn btn-default" href="listar">Listar</a>
			<a class="btn btn-default" href="index">Home</a>
		    
			<form action="/gravar" commandName="cartoriosimples" method="post">
				<input type="hidden" name="id">
				
				<div class="form-group" style="margin-top: 20px">
					<label for="nome">Nome:</label>
					<input id="nome" class="form-control" placeholder="Nome" type="text" name="nome">
					
				</div>
				<input class="btn btn-success" type="submit" value="Salvar" style="margin-top: 20px"> 
		
			</form>
		</div>
	</div>
	
	<script src="http://cdn.jsdelivr.net/webjars/jquery/3.2.1/jquery.min.js" src="@{/webjars/jquery/3.2.1/jquery.min.js}"></script>
</body>
</html>

